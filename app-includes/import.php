...
function cota_insert_family_and_get_id( $data ) {
	...
	$statement = $cota_db->conn->prepare(
	...
	$home_phone       = cota_format_phone( $data['homephone'] );
	...
	$anniversary_date = cota_format_date( $data['annday'] );
...
function cota_insert_member( $family_id, $first_name, $last_name, $cell_phone, $email, $birthday, $baptism ) {
	...
	$statement = $cota_db->conn->prepare( 'INSERT INTO members (family_id, first_name, last_name, cell_phone, email, birthday, baptism) VALUES (?, ?, ?, ?, ?, ?, ?)' );
...
echo '<div class="cota-import-container">';
echo '<h2>Upload CSV File containing Family Import</h2>';
echo '<form class="cota-import" method="post" enctype="multipart/form-data">';
echo '<label>Select CSV File:</label>';
echo '<input type="file" name="csv_file" accept=".csv" required>';
echo '<button class="cota-import" type="submit">Upload & Import</button>';
echo '</form>';
echo '</div>';
echo '</body>';
echo '</html>';
