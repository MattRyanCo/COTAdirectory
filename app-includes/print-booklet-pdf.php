...
$pdf->front_cover( $title, $author, $logo_file ); // Add front cover with logo

// Retrieve and Format Membership Entries
$families        = $cota_db->read_family_database();
$num_families    = $families->num_rows;
$field_positions = $field_widths = $field_info = array();

// Load and insert static pages.
for ( $i = 1; $i <= 3; $i++ ) {
	$pdf->print_chapter( $i, 'intro' . $i . '.txt', '../uploads/intro' . $i . '.txt' );
}
