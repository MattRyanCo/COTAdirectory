...
class Membership_Directory_Printer {
	...
	public function generate_rtf_header() {
	 ...
	 * @return void
	 */
	public function format_family_listings( $families ) {
		...
		for ( $i = 1; $i <= $num_intro_pages; $i++ ) {
			...
			if ( file_exists( $file ) ) {
				if ( $i == $num_intro_pages ) {
					...
					$rtf_content .= $this->format_text( file_get_contents( $file ) ) . '\\pard\\par';
				} else {
					$rtf_content .= $this->format_text( file_get_contents( $file ) ) . '\\pard\\page\\par';
				}
			}
		}
		return $rtf_content;
